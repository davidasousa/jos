...
trap_init(void)
{
    ...
    void th_irq_timer(); // IRQ Timers
    void th_irq_kbd(); // KBD
    void th_irq_serial(); // Serial
    void th_irq_spurious(); // Spurious
    void th_irq_ide(); // IDE
    void th_irq_error();
...
trap_init_percpu(void)
{
    ...
    int pid = thiscpu -> cpu_id;

	// Setup a TSS so that we get the right stack
	// when we trap to the kernel.
	ts.ts_esp0 = (uint32_t)percpu_kstacks[pid] + KSTKSIZE;
	ts.ts_ss0 = GD_KD;
	ts.ts_iomb = sizeof(struct Taskstate);

	// Initialize the TSS slot of the gdt.
	gdt[(GD_TSS0 >> 3) + pid] = SEG16(STS_T32A, (uint32_t) (&ts),
					sizeof(struct Taskstate) - 1, 0);
	gdt[(GD_TSS0 >> 3) + pid].sd_s = 0;
